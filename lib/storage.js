...
async function getUserId() {
  ...
  if (!supabase) return fallbackVisitorId();
 ...
 * @param {boolean} shared - true=共享表(kv_shared), false=个人表(kv_personal)
 */
 ...
 * 向 Supabase 写入数据。
 * @param {string} key - 存储键
 * @param {*} val - 要存储的值（会被序列化为 JSONB）
 * @param {boolean} shared - true=共享表, false=个人表
 */
